    ...
    <form action="{{ route('agendamentos.update', $dados->id) }}" method="POST">
        ...
        <div class="form-row">
            <div class="form-group col-md-6">
                ...
                <input type="text" id="descricao" name="descricao" class="form-control"  value="{{ $dados->descricao }}" required/>
            </div>
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control" id="id_medico" name="id_medico" required>
                    ...
                    @foreach ($dados_medicos as $dado_medico)
                        <option value='{{ $dado_medico->id }}' {{ $dado_medico->id == $dados->id_medico ? 'selected="selected"' : ''}}>{{ $dado_medico->nome }}</option>
                    @endforeach
                </select>
            </div>
